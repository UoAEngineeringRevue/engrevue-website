...
/* This database holds the objects that store all old and new revue data. */

export class RevueData {
    constructor(year) {
        this.coverPicture = this.getCoverPicture(this.year);
        this.bigPicture = this.getCastPicture(this.year);
        this.title = this.getTitle(this.year);
        this.subtitle = this.getSubTitle(this.year);
        this.catchPhrase = this.getCatchPhrase(this.year);
        this.location = this.getLocation(this.year);
        this.director = this.getDirector(this.year);
        this.producer = this.getProducer(this.year);
        this.blurb = this.getBlurb(this.year);
        this.quote = this.getQuote(this.year);
        this.subquote = this.getSubQuote(this.year);

        /* Still need pdf's of programme and also LINK to the watch now page. */
        
    }

    /* Store data for each year in these functions. */
    ...
    getBlurb(year) {
        if (year == '2019') return "When a sudden loss sends the student kingdom into disarray, the scramble for power begins, and an undead threat emerges from the shadows. One thing's for certain; the race for the Iron Loan is a matter of life and debt."; 
